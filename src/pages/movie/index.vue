<template>
  <div class="movie">
    ...
    <div class="tabs">
      ...
      <div class="tab">
        <span v-for="item in tabs" :key="item.id" :class="{ active: current === item.id }" @click="switchTab(item.id)">{{ item.title }}</span>
      </div>
      <div class="search">
    ...
    <div ref="wrapper" class="scroll-wrapper">
      <div class="content">
        <div v-for="item in movieOnInfoData.movieOnInfoList" :key="item.id" class="item">
          <div class="img">
...
import { getMovieOnInfoList, getMoreComingList } from '@/api/index'
...
export default {
    ...
    this.$nextTick(() => {
      ...
      if (!this.scroll) {
        ...
        this.scroll.on('pullingUp', function() {
          const len = that.movieOnInfoData.movieOnInfoList.length
          const total = that.movieOnInfoData.total
          if (len < total) {
            that.getMoreComingList(len)
            this.finishPullUp()
          }
        })
  ...
  methods: {
    ...
    getMovieOnInfoList() {
      const that = this
      const token = ''
      getMovieOnInfoList({ token }).then(res => {
        const { movieIds, total } = res.data
        ...
        that.movieOnInfoData = { movieIds, movieOnInfoList: data, total }
      }).catch(err => {
        console.log(err)
      })
    },
    // 正在热映加载更多
    getMoreComingList(len) {
      const that = this
      const token = ''
      const ids = [...that.movieOnInfoData.movieIds]
      const movieIds = ids.splice(len, 10).join(',')
      getMoreComingList({ token, movieIds }).then(res => {
        const data = res.data.coming.map(item => {
          const img = item.img.replace('w.h', '128.180')
          return {
            id: item.id,
            globalReleased: item.globalReleased,
        ...
        const movieOnInfoList = that.movieOnInfoData.movieOnInfoList.concat(data)
        that.movieOnInfoData = { ...that.movieOnInfoData, movieOnInfoList }
      }).catch(err => {
        console.log(err)
      })
    },
